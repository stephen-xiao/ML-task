...
class NN():
    ...
    def initParameters(self):
        ...
        for i in range(1,self.layer+1):
                ...
                self.parameters['b'+str(i)] = np.random.randn(self.hide_size[i-1],1)
    def printShape(self):
    ...
    def sigmoidgrad(self,x):
        z = np.zeros_like(x)
        z = self.sigmoid(x)*(1-self.sigmoid(x))
        return z
    def backward_propagation(self,lr = 0.0005):
        for i in range(self.layer,0,-1):
            if i == self.layer:
                self.dz[i] = (1/self.m)*(self.a[i]-self.y)*self.sigmoidgrad(self.z[i])/(self.a[i]*(1-self.a[i]))
            else:
                self.dz[i] = np.dot(self.parameters['w'+str(i+1)].T , self.dz[i+1])*self.relugrad(self.z[i])
                #self.dz[i] = np.dot(self.parameters['w'+str(i+1)].T , self.dz[i+1])*(1-np.power(self.a[i],2))
            dw = np.dot(self.dz[i],self.a[i-1].T)
            db = np.dot(self.dz[i],np.ones((self.dz[i].shape[1],1)))
            self.parameters['w'+str(i)] -= lr*dw
            self.parameters['b'+str(i)] -= lr*db
    def predict(self,x):
        y_pre = np.zeros_like(x)
        y_pre[x>0.5] = 1
        y_pre[x<0.5] = 0
        return y_pre
    def model(self,iteration = 100000,lr = 0.0005,printloss = True):
        self.initParameters()
        self.printShape()
        losses = []
        loss = 0
